...
def enhancedFeatureExtractorDigit(datum):
    ...
    1. Feature 'upper':
        'upper' = 1 if upper half has more points for numbers like 9 etc.
    2. Feature 'left':
        'left' = 1 if left half has more points for numbers like 6
    3. Feature "HorizontalSymmetry":
        'hs' = 1 if left and right side are symmetry (30% points coincide) for numbers like 8,0,1 etc.
    4. Feature "regions":
        'regions{i}' = 1 if there are exactly i connected regions in image
    5. Feature "empty":
        'empty{i}' = 0 if line i is white
    6. Feature "hole":
        'hole{i}' == 1 if line i has a hole i.e for numbers 6,8 etc.
    ...
    def getPartitionNum(pixels):
        ...
        def neighbours(x, y):
            ...
            neigh = set()
            for x,y in candidates_neighbours:
                if pointInImage(x,y):
                    neigh.add((x,y))
            return neigh
        ...
        def bfs(explored_set, start_point, spaceIndex, partition):
            queue = [start_point]
            if pointOnEdge(*start_point):
                showEnqueue = pointOnEdge
            else:
                showEnqueue = pointNotOnEdge
            while len(queue) > 0:
        ...
        while not isComplete:
            ...
            spaceIndex += 1
            isComplete, startPoint = partitionIsComplete(partition)
        counter = Counter(partition.values()).items()
        length = 0
        for c in counter:
            if c[1]>3:
                length += 1
    ...
    left = 0
    right = 0
    for row in xrange(DIGIT_DATUM_HEIGHT/2):
        for col in xrange(DIGIT_DATUM_WIDTH):
            if pixels[row][col] >0:
                upper += pixels[row][col]

    for row in xrange(DIGIT_DATUM_HEIGHT/2,DIGIT_DATUM_HEIGHT):
        for col in xrange(DIGIT_DATUM_WIDTH):
            if pixels[row][col] >0:
                lower += pixels[row][col]

    for row in xrange(DIGIT_DATUM_WIDTH):
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        for x in xrange(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y) == 0:
                pixelsInbinary.append(False)
            else:
                pixelsInbinary.append(True)
        numBlackPixels = sum(pixelsInbinary)
        addFeature('empty'+str(y), numBlackPixels > 0, 3)
        if numBlackPixels:
            leftEdge = (DIGIT_DATUM_WIDTH-1)-pixelsInbinary[::-1].index(True)
            rightEdge = pixelsInbinary.index(True)
        else:
             leftEdge = 0
             rightEdge = 0
        ...
        hs = 0
        for x in xrange(DIGIT_DATUM_WIDTH / 2):
            hs += (pixels[x][y] > 0 and pixels[DIGIT_DATUM_WIDTH - 1 - x][y] > 0)
...
def enhancedPacmanFeatures(state, action):
    ...
    pacPos = successorState.getPacmanPosition()

    features['remainingFood'] = successorState.getNumFood()

    foodList = successorState.getFood().asList()
    closestFood = 0
    foodDist = []
    for fPos in foodList:
       foodDist.append(util.manhattanDistance(pacPos,fPos))
    if len(foodDist) !=0:
    ...
    ghostList = successorState.getGhostPositions()
    closestGhost = 0
    ghostDist = []
    for gPos in ghostList:
        ghostDist.append(util.manhattanDistance(pacPos,gPos))
    if len(ghostDist) !=0:

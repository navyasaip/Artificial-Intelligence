...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #for calculating beliefs
        self.beliefs = util.Counter()
        for p in self.legalPositions: self.beliefs[p] = 1.0
        self.beliefs.normalize()

        # uniform prior distribution of particles
        self.particles =[]
        for i in range(self.numParticles):
            self.particles += [random.choice(self.legalPositions)]

        # initializing particleWeights to 1 i.e count of duplicates of particles in the list
        self.particleWeights = util.Counter()
        for p in self.particles: self.particleWeights[p] += 1.0
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = self.particleWeights[p]*emissionModel[trueDistance]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #for calculating beliefs
        self.beliefs = util.Counter()
        for p in self.particles: self.beliefs[p] += 1.0
        self.beliefs.normalize()
